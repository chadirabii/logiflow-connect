...
export default function AdminRequestsPage() {
  ...
  const filtered = bookings
    ...
    .filter(b => !search || b.reference_number.toLowerCase().includes(search.toLowerCase()) || (b.company || '').toLowerCase().includes(search.toLowerCase()));

  const handleUpdateStatus = async (id: string, status: string) => {
    await updateStatus.mutateAsync({ id, status });
    toast.success(`Statut mis à jour → ${STATUS_LABELS[status as keyof typeof STATUS_LABELS]}`);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent><SelectItem value="all">Tous les statuts</SelectItem>{Object.entries(STATUS_LABELS).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent>
          </Select>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>{filtered.map(b => (
                <tr key={b.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground text-xs">{b.origin_port} → {b.destination_port}</td>
                  ...
                  <td className="px-4 py-3"><div className="flex gap-1">
                    {b.status === 'submitted' && <Button size="sm" variant="ghost" onClick={() => handleUpdateStatus(b.id, 'under_review')} className="text-warning h-7 px-2">Examiner</Button>}
                    {b.status === 'under_review' && (<><Button size="sm" variant="ghost" onClick={() => handleUpdateStatus(b.id, 'approved')} className="text-success h-7 px-2"><Check className="h-3 w-3 mr-1" />Approuver</Button><Button size="sm" variant="ghost" onClick={() => handleUpdateStatus(b.id, 'rejected')} className="text-destructive h-7 px-2"><X className="h-3 w-3 mr-1" />Refuser</Button></>)}
                    {b.status === 'approved' && <Button size="sm" variant="ghost" onClick={() => handleUpdateStatus(b.id, 'confirmed')} className="text-accent h-7 px-2">Confirmer</Button>}
                    {b.status === 'confirmed' && <Button size="sm" variant="ghost" onClick={() => handleUpdateStatus(b.id, 'in_transit')} className="text-accent h-7 px-2">Expédier</Button>}
                  </div></td>
                </tr>
              ))}</tbody>
            </table>
