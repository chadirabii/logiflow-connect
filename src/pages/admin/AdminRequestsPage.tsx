...
import { bookingRequests, STATUS_LABELS, type BookingStatus } from '@/data/mockData';
                  ...
                  <tr key={b.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        {b.status === 'submitted' && (
                          <>
                            <Button size="sm" variant="ghost" onClick={() => updateStatus(b.id, 'under_review')} className="text-warning h-7 px-2">Examiner</Button>
                          </>
                        )}
                        {b.status === 'under_review' && (
                          <>
                            <Button size="sm" variant="ghost" onClick={() => updateStatus(b.id, 'approved')} className="text-success h-7 px-2"><Check className="h-3 w-3 mr-1" />Approuver</Button>
                            <Button size="sm" variant="ghost" onClick={() => updateStatus(b.id, 'rejected')} className="text-destructive h-7 px-2"><X className="h-3 w-3 mr-1" />Refuser</Button>
                          </>
                        )}
                        {b.status === 'approved' && (
                          <Button size="sm" variant="ghost" onClick={() => updateStatus(b.id, 'confirmed')} className="text-accent h-7 px-2">Confirmer</Button>
                        )}
                        {b.status === 'confirmed' && (
                          <Button size="sm" variant="ghost" onClick={() => updateStatus(b.id, 'in_transit')} className="text-accent h-7 px-2">Expédier</Button>
                        )}
                      </div>
                    </td>
                  </tr>
