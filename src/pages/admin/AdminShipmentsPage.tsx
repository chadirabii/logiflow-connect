...
export default function AdminShipmentsPage() {
  const { shipments, updateShipment } = useStore();
  ...
  const startEdit = (id: string) => {
    ...
    if (s) { setEditForm({ status: s.status, currentLocation: s.currentLocation, currentPort: s.currentPort, estimatedArrival: s.estimatedArrival }); setEditId(id); }
  };

  const saveEdit = () => {
    const s = shipments.find(sh => sh.id === editId);
    if (s) {
      updateShipment(s.id, { status: editForm.status as BookingStatus, currentLocation: editForm.currentLocation, currentPort: editForm.currentPort, estimatedArrival: editForm.estimatedArrival, updatedAt: new Date().toISOString().split('T')[0] });
              ...
              <tbody>
                {filtered.map(s => (
                  <tr key={s.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3"><div className="flex items-center gap-1"><MapPin className="h-3 w-3 text-accent" /><span className="text-xs text-muted-foreground">{s.currentLocation}</span></div></td>
