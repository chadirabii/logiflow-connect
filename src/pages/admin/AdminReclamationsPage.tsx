...
import { RECLAMATION_STATUS_LABELS, RECLAMATION_STATUS_COLORS, RECLAMATION_PRIORITY_LABELS, RECLAMATION_PRIORITY_COLORS, type Reclamation, type ReclamationStatus } from '@/data/mockData';
...
export default function AdminReclamationsPage() {
  const { reclamations, updateReclamation } = useStore();
  ...
  const handleUpdate = () => {
    if (selected) {
      updateReclamation(selected.id, { status: newStatus, adminResponse: response || selected.adminResponse, updatedAt: new Date().toISOString().split('T')[0] });
      setSelected(null);
      setResponse('');
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-warning/10 flex items-center justify-center"><AlertTriangle className="h-5 w-5 text-warning" /></div><div><p className="text-sm text-muted-foreground">Ouvertes</p><p className="text-xl font-bold text-foreground">{openCount}</p></div></div>
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-accent/10 flex items-center justify-center"><Clock className="h-5 w-5 text-accent" /></div><div><p className="text-sm text-muted-foreground">En traitement</p><p className="text-xl font-bold text-foreground">{inProgressCount}</p></div></div>
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-success/10 flex items-center justify-center"><CheckCircle className="h-5 w-5 text-success" /></div><div><p className="text-sm text-muted-foreground">Résolues</p><p className="text-xl font-bold text-foreground">{resolvedCount}</p></div></div>
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3"><div className="h-10 w-10 rounded-lg bg-muted flex items-center justify-center"><XCircle className="h-5 w-5 text-muted-foreground" /></div><div><p className="text-sm text-muted-foreground">Fermées</p><p className="text-xl font-bold text-foreground">{closedCount}</p></div></div>
        </div>
        <div className="flex items-center gap-2"><Label className="text-sm">Filtrer :</Label><Select value={filter} onValueChange={v => setFilter(v as ReclamationStatus | 'all')}><SelectTrigger className="w-48"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Toutes</SelectItem><SelectItem value="open">Ouvertes</SelectItem><SelectItem value="in_progress">En traitement</SelectItem><SelectItem value="resolved">Résolues</SelectItem><SelectItem value="closed">Fermées</SelectItem></SelectContent></Select></div>
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted/50"><tr><th className="text-left p-3 font-medium text-muted-foreground">Client</th><th className="text-left p-3 font-medium text-muted-foreground">Sujet</th><th className="text-left p-3 font-medium text-muted-foreground">Réf. commande</th><th className="text-left p-3 font-medium text-muted-foreground">Priorité</th><th className="text-left p-3 font-medium text-muted-foreground">Statut</th><th className="text-left p-3 font-medium text-muted-foreground">Date</th><th className="text-left p-3 font-medium text-muted-foreground">Actions</th></tr></thead>
            <tbody className="divide-y divide-border">
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-muted/30">
                  <td className="p-3 text-foreground">{r.clientName}</td><td className="p-3 text-foreground">{r.subject}</td><td className="p-3 text-muted-foreground">{r.bookingRef || '—'}</td>
                  <td className="p-3"><span className={cn('inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium', RECLAMATION_PRIORITY_COLORS[r.priority])}>{RECLAMATION_PRIORITY_LABELS[r.priority]}</span></td>
                  <td className="p-3"><span className={cn('inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium', RECLAMATION_STATUS_COLORS[r.status])}>{RECLAMATION_STATUS_LABELS[r.status]}</span></td>
                  <td className="p-3 text-muted-foreground">{r.createdAt}</td>
                  <td className="p-3"><Button size="sm" variant="outline" onClick={() => { setSelected(r); setNewStatus(r.status); setResponse(r.adminResponse || ''); }}>Traiter</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
          <DialogContent className="sm:max-w-lg">
            <DialogHeader><DialogTitle>Réclamation : {selected?.subject}</DialogTitle></DialogHeader>
            {selected && (
              <div className="space-y-4">
                <div className="text-sm space-y-1"><p><span className="font-medium text-muted-foreground">Client :</span> {selected.clientName}</p>{selected.bookingRef && <p><span className="font-medium text-muted-foreground">Commande :</span> {selected.bookingRef}</p>}</div>
