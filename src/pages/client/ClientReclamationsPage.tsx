    ...
    <ClientLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-heading font-bold text-foreground">Mes réclamations</h1>
            <p className="text-muted-foreground">Soumettez et suivez vos réclamations.</p>
          </div>
            ...
            <DialogContent className="sm:max-w-lg">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <Select value={form.bookingRef} onValueChange={v => setForm({ ...form, bookingRef: v })}>
                ...
                <div>
                  <Label>Sujet *</Label>
                  <Input value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} placeholder="Résumé de votre réclamation" className="mt-1.5" required />
                </div>
                <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3">
            ...
            <div><p className="text-sm text-muted-foreground">Ouvertes</p><p className="text-xl font-bold text-foreground">{myReclamations.filter(r => r.status === 'open').length}</p></div>
          </div>
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3">
            <div className="h-10 w-10 rounded-lg bg-accent/10 flex items-center justify-center"><Clock className="h-5 w-5 text-accent" /></div>
            <div><p className="text-sm text-muted-foreground">En traitement</p><p className="text-xl font-bold text-foreground">{myReclamations.filter(r => r.status === 'in_progress').length}</p></div>
          </div>
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3">
            ...
            <div><p className="text-sm text-muted-foreground">Résolues</p><p className="text-xl font-bold text-foreground">{myReclamations.filter(r => ['resolved', 'closed'].includes(r.status)).length}</p></div>
          </div>
        </div>
