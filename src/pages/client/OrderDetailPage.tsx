...
import { bookingRequests, shipments, trackingEvents, documents as allDocs, STATUS_LABELS } from '@/data/mockData';
...
export default function OrderDetailPage() {
  ...
  const docs = allDocs.filter(d => d.bookingId === booking.id);
    ...
    <ClientLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-heading font-bold text-foreground">{booking.referenceNumber}</h1>
            <p className="text-muted-foreground">{booking.originPort} → {booking.destinationPort}</p>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            {booking.specialInstructions && (
              <div className="mt-2">
                ...
                <p className="text-sm font-medium text-card-foreground mt-1">{booking.specialInstructions}</p>
              </div>
            )}
          </div>
          ...
          {shipment && (
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex items-center gap-2"><Package className="h-4 w-4 text-accent" /><span className="text-muted-foreground">Conteneur :</span><span className="font-medium text-card-foreground">{shipment.containerNumber}</span></div>
                ...
                <div key={d.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{d.size} • {d.createdAt}</p>
                    </div>
                  </div>
