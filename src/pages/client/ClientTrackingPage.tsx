...
export default function ClientTrackingPage() {
  const { user } = useAuth();
  const { shipments, trackingEvents } = useStore();
          ...
          <div className="space-y-3">
            ...
            {myShipments.map(s => (
              <div key={s.id} onClick={() => setSelected(s.id)} className={`rounded-xl border p-4 cursor-pointer transition-all ${activeShipment?.id === s.id ? 'border-accent bg-accent/5 shadow-sm' : 'border-border bg-card hover:border-accent/30'}`}>
                <div className="flex items-center justify-between mb-2"><span className="font-heading font-semibold text-sm text-card-foreground">{s.referenceNumber}</span><StatusBadge status={s.status} /></div>
                <p className="text-xs text-muted-foreground">{s.originPort} → {s.destinationPort}</p>
                <div className="flex items-center gap-1 mt-2"><MapPin className="h-3 w-3 text-accent" /><span className="text-xs text-muted-foreground">{s.currentLocation}</span></div>
              </div>
            ))}
            ...
            {activeShipment ? (
              <>
                <div className="rounded-xl border border-border bg-card p-6">
                  <div className="flex items-center justify-between mb-4"><h2 className="text-xl font-heading font-bold text-card-foreground">{activeShipment.referenceNumber}</h2><StatusBadge status={activeShipment.status} className="text-sm px-4 py-1" /></div>
                ...
                <div className="rounded-xl border border-border bg-card p-6"><h3 className="font-heading font-semibold text-card-foreground mb-4">Historique</h3><TrackingTimeline events={events} /></div>
              </>
            ) : (
              <div className="rounded-xl border border-border bg-card p-12 text-center"><Ship className="h-12 w-12 text-muted-foreground mx-auto mb-3" /><p className="text-muted-foreground">Sélectionnez une expédition pour voir le suivi.</p></div>
            )}
          </div>
        </div>
      </div>
