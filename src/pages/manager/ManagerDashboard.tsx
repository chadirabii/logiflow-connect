...
import { useAllBookings, useAllReclamations } from '@/hooks/useSupabaseData';
      ...
      <div className="space-y-6">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <div className="space-y-3">{bookings.slice(0, 5).map(b => (<div key={b.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50"><div className="flex-1 min-w-0"><p className="text-sm font-medium text-card-foreground">{b.reference_number}</p><p className="text-xs text-muted-foreground truncate">{b.company} — {b.origin_port} → {b.destination_port}</p></div><StatusBadge status={b.status} /></div>))}</div>
          </div>
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <div className="space-y-3">{reclamations.slice(0, 5).map(r => (<div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50"><div className="flex-1 min-w-0"><p className="text-sm font-medium text-card-foreground">{r.subject}</p></div><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${RECLAMATION_STATUS_COLORS[r.status as keyof typeof RECLAMATION_STATUS_COLORS]}`}>{RECLAMATION_STATUS_LABELS[r.status as keyof typeof RECLAMATION_STATUS_LABELS]}</span></div>))}</div>
          </div>
        </div>
      </div>
    </ManagerLayout>
