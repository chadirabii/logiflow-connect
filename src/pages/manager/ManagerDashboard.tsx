...
import { bookingRequests, reclamations } from '@/data/mockData';
import { Package, AlertTriangle, BarChart3, CheckCircle } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { RECLAMATION_STATUS_LABELS, RECLAMATION_STATUS_COLORS, RECLAMATION_PRIORITY_COLORS, RECLAMATION_PRIORITY_LABELS } from '@/data/mockData';
...
export default function ManagerDashboard() {
  ...
  const totalOrders = bookingRequests.length;

  return (
    <ManagerLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-heading font-bold text-foreground">Dashboard Manager</h1>
          <p className="text-muted-foreground">Vue d'ensemble de votre activité.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard title="Total commandes" value={totalOrders} icon={Package} />
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button variant="ghost" size="sm" onClick={() => navigate('/manager/orders')} className="text-accent">Voir tout</Button>
            </div>
              ...
              {bookingRequests.slice(0, 5).map(b => (
                <div key={b.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{b.company} — {b.originPort} → {b.destinationPort}</p>
                  </div>
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-heading font-semibold text-card-foreground">Réclamations récentes</h3>
              <Button variant="ghost" size="sm" onClick={() => navigate('/manager/reclamations')} className="text-accent">Voir tout</Button>
            </div>
            <div className="space-y-3">
              {reclamations.slice(0, 5).map(r => (
                <div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  ...
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${RECLAMATION_STATUS_COLORS[r.status]}`}>
