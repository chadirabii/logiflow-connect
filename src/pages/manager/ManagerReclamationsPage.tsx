...
export default function ManagerReclamationsPage() {
  ...
  const handleRespond = async () => {
    ...
    const result = await updateReclamation(selected.id, {
      adminResponse: response,
      status: "in_progress",
      updatedAt: new Date().toISOString().split("T")[0],
    });
    if (!result.success) {
      toast.error(result.error ?? "Réponse non enregistrée");
      return;
    }
  ...
  const handleStatusChange = async (
    ...
    if (!result.success) {
      toast.error(result.error ?? "Statut non mis à jour");
      return;
    }
                ...
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {r.clientName} {r.bookingRef && `• ${r.bookingRef}`}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${RECLAMATION_PRIORITY_COLORS[r.priority]}`}
                    >
                      {RECLAMATION_PRIORITY_LABELS[r.priority]}
                    </span>
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${RECLAMATION_STATUS_COLORS[r.status]}`}
                    >
            ...
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    handleStatusChange(selected.id, v as ReclamationStatus)
                  }
                >
